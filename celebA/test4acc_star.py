...
save_path1= '/raid/zhaoyihao/code3/ILGAN/stargan-celeba'
...
#    tl.files.exists_or_mkdir(save_path1+'/img_vacant_b', verbose=False)
    tl.files.exists_or_mkdir(save_path1+'/black', verbose=False)
    tl.files.exists_or_mkdir(save_path1+'/blond', verbose=False)
    tl.files.exists_or_mkdir(save_path1+'/brown', verbose=False)
    tl.files.exists_or_mkdir(save_path1+'/male', verbose=False)
    tl.files.exists_or_mkdir(save_path1+'/female', verbose=False)
    tl.files.exists_or_mkdir(save_path1+'/blond_female', verbose=False)
    tl.files.exists_or_mkdir(save_path1+'/black_male', verbose=False)
...
#        raise Exception("missing Ec model")
    ...
    change_label = np.load('test_acc/Xa_r_label_vacant.npy')
    change_label = change_label.astype(np.float32)
    X_path = np.load('test_acc//Xa_vacant.npy')
    X_num = change_label.shape[0]
    X_img = np.ones([X_num, img_size, img_size, 3], dtype=np.float32)
    change_name_list = []

    X_ans_label = np.load('test_acc/Xa_ans_label_vacant.npy')
    X_ans_label = X_ans_label.astype(np.float32)

    for i in range(X_num):
        X_img[i] = _map_fn2('/raid/zhaoyihao/code3/ILGAN/stargan-celeba/'+str(X_path[i][:-4])+'_pr.jpg')
...
#            tl.visualize.save_images(fake_X[j:j+1], [1,1], save_path1 + '/img_vacant_b/'+str(X_path[X_idx]))
            if list(X_pr_label[j][0:3])==[1,0,0]:
                tl.visualize.save_images(X_test[j:j+1], [1,1], save_path1 + '/black/'+str(X_path[X_idx][:-4])+'_pr.jpg')
            elif list(X_pr_label[j][0:3])==[0,1,0]:
                tl.visualize.save_images(X_test[j:j+1], [1,1], save_path1 + '/blond/'+str(X_path[X_idx][:-4])+'_pr.jpg')
            elif list(X_pr_label[j][0:3])==[0,0,1]:
                tl.visualize.save_images(X_test[j:j+1], [1,1], save_path1 + '/brown/'+str(X_path[X_idx][:-4])+'_pr.jpg')
            if list(X_pr_label[j][3:5])==[1,0]:
                tl.visualize.save_images(X_test[j:j+1], [1,1], save_path1 + '/male/'+str(X_path[X_idx][:-4])+'_pr.jpg')
            elif list(X_pr_label[j][3:5])==[0,1]:
                tl.visualize.save_images(X_test[j:j+1], [1,1], save_path1 + '/female/'+str(X_path[X_idx][:-4])+'_pr.jpg')
            if list(X_pr_label[j])==[1,0,0,1,0]:
                tl.visualize.save_images(X_test[j:j+1], [1,1], save_path1 + '/black_male/'+str(X_path[X_idx][:-4])+'_pr.jpg')
            if list(X_pr_label[j])==[0,1,0,0,1]:
                tl.visualize.save_images(X_test[j:j+1], [1,1], save_path1 + '/blond_female/'+str(X_path[X_idx][:-4])+'_pr.jpg')
