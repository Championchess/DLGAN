...
class FLAGS2(object):
    def __init__(self):
        ...
        self.test_samples = 16 # sample num for testing
...
def get_dataset(img_size=128, hair=0, male=1, age_w=0):  # hair:0/1/2 black/blond/brown     male: 0/1 female/male  age_w: 0/1 young/old
    ...
    test_list = [(i1, i2, i3) for (i1, i2, i3) in zip(img_list, match_vec_list, img_name_list)]
    ...
    test_ds = tf.data.Dataset.from_generator(generator_test, output_types=(tf.string, tf.float32, tf.string))
    # test_ds = test_ds.shuffle(buffer_size=flags2.shuffle_buffer_size)
    test_ds = test_ds.map(lambda x1, x2, x3: (_map_fn(x1), x2, x3), num_parallel_calls=multiprocessing.cpu_count())
    test_ds = test_ds.batch(flags2.test_samples)
    test_ds = test_ds.prefetch(buffer_size=4)
    # print(test_ds)
    return test_ds, dataset_len


black_male_ds, black_male_len = get_dataset(128, 0, 1)
blond_male_ds, blond_male_len = get_dataset(128, 1, 1)
brown_male_ds, brown_male_len = get_dataset(128, 2, 1)

black_female_ds, black_female_len = get_dataset(128, 0, 0)
blond_female_ds, blond_female_len = get_dataset(128, 1, 0)
brown_female_ds, brown_female_len = get_dataset(128, 2, 0)
